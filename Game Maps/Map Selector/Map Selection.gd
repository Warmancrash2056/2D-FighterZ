...
func _process(delta):
	...
	match MapCall:
		Map.Galvin:
			...
			Galvin.visible = true
			Artic.visible = false
			if Input.is_action_just_pressed(player_1_controller.right) or Input.is_action_just_pressed(player_2_controller.right):
				MapCall = Map.Artic
				Galvin.visible = false
				Artic.visible = true
				
			elif Input.is_action_just_pressed(player_1_controller.left) or Input.is_action_just_pressed(player_2_controller.left):
				MapCall = Map.Artic
				Galvin.visible = false
				Artic.visible = true
